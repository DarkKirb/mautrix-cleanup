"""
Cleaning code for mautrix_signal
"""
...
engine = create_engine(config.MAUTRIX_SIGNAL_DB)


def delete_media(mxurls: Iterable[str]):
    """
    Delete media from mautrix_signal database
...
def known_media() -> Iterable[str]:
    """
    Get all known media urls from mautrix_signal database
...
def delete_missing_messages(existing_messages: Iterable[str]):
    """
    Delete all messages from mautrix_signal database that are not in existing_messages
    ...
    with engine.connect() as conn:
        with conn.begin():
            ...
            conn.execute(text("DELETE FROM disappearing_message WHERE NOT EXISTS (SELECT 1 FROM current_messages WHERE disappearing_message.mxid = current_messages.mxid)"))
