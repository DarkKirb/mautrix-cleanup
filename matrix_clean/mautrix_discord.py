"""
Cleaning code for mautrix_discord
"""
...
engine = create_engine(config.MAUTRIX_DISCORD_DB)


def delete_media(mxurls: Iterable[str]):
    """
    Delete media from mautrix_discord database
    ...
    with engine.connect() as conn:
        with conn.begin():
            for mxurl in tqdm(mxurls):
                ...
                conn.execute(text("UPDATE guild SET avatar_url = NULL, avatar_set = 'f' WHERE avatar_url IN (:mxurl)"), {"mxurl": mxurl})
                conn.execute(text("UPDATE puppet SET avatar_url = NULL, avatar_set = 'f' WHERE avatar_url IN (:mxurl)"), {"mxurl": mxurl})
                conn.execute(text("DELETE FROM discord_file WHERE mxc IN (:mxurl)"), {"mxurl": mxurl})


def known_media() -> Iterable[str]:
    """
    Get all known media urls from mautrix_discord database
    :return: list of media urls
    """
    with engine.connect() as conn:
        with conn.begin():
            yield from map(lambda x: x[0], conn.execute(text("SELECT mxc FROM discord_file WHERE mxc IS NOT NULL")).fetchall())


def delete_missing_messages(existing_messages: Iterable[str]):
    """
    Delete all messages from mautrix_discord database that are not in existing_messages
    ...
    with engine.connect() as conn:
        with conn.begin():
            ...
            conn.execute(text("DELETE FROM thread WHERE NOT EXISTS (SELECT 1 FROM current_messages WHERE root_msg_mxid = mxid)"))
            conn.execute(text("DELETE FROM thread WHERE NOT EXISTS (SELECT 1 FROM current_messages WHERE creation_notice_mxid = mxid)"))
            conn.execute(text("DROP TABLE IF EXISTS current_messages"))
